...
class ServiceClass
{
    ...
    public function process($search, $itemPerPage, $page)
    {
        ...
        $dynamics .= " AND clientid IN (SELECT clientid FROM treatmentsoa tsoatbl where tsoatbl.soaid IN (SELECT soaid FROM treatmentsub where treatment like '%orthodontic%'))"; 
