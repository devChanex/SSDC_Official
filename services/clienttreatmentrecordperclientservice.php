...
class ServiceClass
{
    ...
    public function loadClientTreatment($fromdate, $todate, $clientname)
    {
        ...
        $query = "SELECT tsoa.soaid, cp.clientid, cp.lname, cp.fname, cp.mdname, tsoa.dentist, tsub.treatment, tsub.details, tsub.remarks,  tsub.price, tsoa.date, tsoa.time FROMclientprofilecp INNER JOIN treatmentsub tsub ON tsub.clientid = cp.clientid INNER JOIN treatmentsoa tsoa ON tsoa.soaid = tsub.soaid WHERE (tsoa.date BETWEEN :a AND :b) AND CONCAT(cp.lname,', ',cp.fname,' ' ,cp.mdname) LIKE '%" . $clientname . "%'";
